...
/*
Time Complexity:
The time complexity of this QuickSort implementation is O(N log N) on average, where N is the size of the array. This occurs because the array is divided into roughly equal halves in each recursive call. However, in the worst case (e.g., when the pivot is always the smallest or largest element, such as in a sorted or reverse-sorted array), the time complexity degrades to O(N²). 
